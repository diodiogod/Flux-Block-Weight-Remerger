...
:: Run the main script
echo Running adjust_tensors.py...
python adjust_tensors.py
if %ERRORLEVEL% NEQ 0 (
    echo An error occurred while running adjust_tensors.py. Please check the script for errors.
    echo %date% %time% - Error running adjust_tensors.py >> %LOGFILE%
    pause
    exit /b 1
)
